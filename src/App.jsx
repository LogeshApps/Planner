...
// ─── Storage Helpers ───────────────────────────────────────────────
const STORAGE_KEY = "cdn_app_data_v1";
...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .nav-item {
    ...
    cursor: pointer; transition: all var(--transition); color: var(--text2); font-size: 0.9rem; font-weight: 500;
    border: 1px solid transparent; user-select: none;
  }
  .nav-item:hover { background: var(--surface2); color: var(--text); }
  .nav-item.active { background: var(--accent-dim); color: var(--accent2); border-color: rgba(99,102,241,0.2); }
  .nav-item .nav-icon { flex-shrink: 0; opacity: 0.8; }
  ...
  .user-badge {
    display: flex; align-items: center; gap: 8px; padding: 6px 12px; background: var(--surface2);
    border-radius: 99px; border: 1px solid var(--border); font-size: 0.85rem;
  }
  ...
  .btn {
    ...
    transition: all var(--transition); border: 1px solid transparent; white-space: nowrap; user-select: none;
  }
  ...
  input, select, textarea {
    ...
    color: var(--text); font-family: var(--font); font-size: 0.9rem; padding: 9px 12px;
    transition: border-color var(--transition), box-shadow var(--transition); width: 100%; outline: none;
  }
  ...
  /* PDF Preview */
  .pdf-preview {
    background: white; color: #111; padding: 40px; font-family: 'DM Sans', sans-serif;
    border-radius: var(--radius); max-width: 600px; margin: 0 auto; line-height: 1.6;
  }
  ...
  .toast {
    ...
    padding: 12px 16px; font-size: 0.875rem; box-shadow: var(--shadow); min-width: 240px;
...
function PersonForm({ initial, type, onSave, onClose }) {
  ...
  return (
    <Modal title={`${initial ? 'Edit' : 'Add'} ${type}`} onClose={onClose} footer={<>
      ...
      <button className="btn btn-primary" onClick={handleSave}>Save {type}</button>
    </>}>
      <div className="form-grid">
        <div className="form-group full">
          <label>{type} Name *</label>
          <input value={form.name} onChange={e => set('name', e.target.value)} placeholder={`Enter ${type.toLowerCase()} name`} autoFocus />
        </div>
        ...
        {type === 'Worker' && (
          <div className="form-group">
            ...
            <input value={form.workType} onChange={e => set('workType', e.target.value)} placeholder="e.g. Carpenter" />
          </div>
        )}
        {type === 'Customer' && (
          <div className="form-group full">
    ...
    <div>
      <div className="section-header">
        <div>
          ...
          <div className="text-muted">{data.length} total</div>
        </div>
      ...
      <div className="toolbar">
        <div className="search-wrap">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder={`Search ${type === 'customer' ? 'customers' : 'workers'}...`} />
        </div>
      </div>
        ...
        <table>
          <thead>
            <tr>
              <th>Name</th>
              <th>Phone</th>
              <th>{type === 'customer' ? 'Address' : 'Work Type'}</th>
              <th>Credits</th>
              <th>Debits</th>
              <th>Balance</th>
              <th>Actions</th>
          ...
          <tbody>
            {filtered.length === 0 && (
              <tr><td colSpan={7}><div className="empty-state"><div className="empty-icon"><Icon name={type === 'customer' ? 'customers' : 'workers'} size={28} /></div><div className="empty-title">No {type === 'customer' ? 'customers' : 'workers'} found</div><div className="empty-sub">Add your first {type} to get started</div></div></td></tr>
            )}
            {filtered.map(p => {
                ...
                <tr key={p.id}>
                  ...
                  <td>
                    <div className="flex gap-2">
                      <button className="btn btn-icon btn-secondary btn-sm" title="View History" onClick={() => onViewHistory(p)}><Icon name="history" size={14} /></button>
                      <button className="btn btn-icon btn-secondary btn-sm" title="Edit" onClick={() => onEdit(p)}><Icon name="edit" size={14} /></button>
                      <button className="btn btn-icon btn-danger btn-sm" title="Delete" onClick={() => setDeleteTarget(p)}><Icon name="trash" size={14} /></button>
                    </div>
                  </td>
                </tr>
              );
      ...
      {deleteTarget && (
        <ConfirmDialog
          message={`Delete "${deleteTarget.name}"? This will not delete their transactions.`}
...
function TransactionForm({ type, customers, workers, initial, onSave, onClose }) {
  const [form, setForm] = useState(initial || {
    date: today(), personType: 'customer', personId: '', amount: '', reason: '', paymentMode: 'cash', refNo: ''
  });
  const [error, setError] = useState('');
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));
  ...
  return (
    <Modal title={`${initial ? 'Edit' : 'New'} ${type === 'credit' ? 'Credit' : 'Debit'} Note`} onClose={onClose} footer={<>
      <button className="btn btn-secondary" onClick={onClose}>Cancel</button>
      <button className={`btn ${type === 'credit' ? 'btn-success' : 'btn-danger'}`} onClick={handleSave}>
        {initial ? 'Update' : 'Save'} {type === 'credit' ? 'Credit' : 'Debit'} Note
      </button>
    </>}>
      {error && <div className="alert alert-error">{error}</div>}
      <div className="form-grid">
        ...
        <div className="form-group">
          ...
          <select value={form.personType} onChange={e => { set('personType', e.target.value); set('personId', ''); }}>
            <option value="customer">Customer</option>
            <option value="worker">Worker</option>
          </select>
        </div>
        <div className="form-group full">
          <label>{form.personType === 'customer' ? 'Customer' : 'Worker'} *</label>
        ...
        <div className="form-group">
          ...
          <input value={form.refNo} onChange={e => set('refNo', e.target.value)} placeholder="TXN-001" />
        </div>
        <div className="form-group full">
          <label>{type === 'credit' ? 'Reason / Description' : 'Description'}</label>
          <textarea value={form.reason} onChange={e => set('reason', e.target.value)} placeholder="Enter description or reason..." rows={2} />
        </div>
      </div>
    </Modal>
  );
      ...
      <div id="pdf-preview-content" className="pdf-preview">
        <div className="pdf-header">
          <div>
            ...
            {companyAddress && <div style={{ color: '#6b7280', fontSize: '0.85rem' }}>{companyAddress}</div>}
            {companyPhone && <div style={{ color: '#6b7280', fontSize: '0.85rem' }}>{companyPhone}</div>}
          </div>
          <div>
            <div className={`pdf-note-type ${isCredit ? 'credit' : 'debit'}`}>{isCredit ? 'CREDIT NOTE' : 'DEBIT NOTE'}</div>
          </div>
        </div>
        ...
        <div style={{ margin: '16px 0', paddingTop: 16, borderTop: '1px solid #e5e7eb' }}>
          <div className="pdf-row">
            <div className="pdf-key">Amount</div>
            <div className={`pdf-val pdf-amount ${isCredit ? 'credit' : 'debit'}`}>{fmt(transaction.amount)}</div>
          </div>
        </div>
    ...
    <div>
      <div className="section-header">
        <div>
          ...
          <div className="text-muted">{filtered.length} records</div>
        </div>
      </div>
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 12 }}>
        <div className="card-sm" style={{ flex: 1, minWidth: 140 }}>
          <div style={{ fontSize: '0.75rem', color: 'var(--text3)', marginBottom: 4 }}>Filtered Credits</div>
          <div style={{ color: 'var(--green)', fontWeight: 700, fontFamily: 'var(--font-display)' }}>{fmt(totalCredit)}</div>
        </div>
        <div className="card-sm" style={{ flex: 1, minWidth: 140 }}>
          <div style={{ fontSize: '0.75rem', color: 'var(--text3)', marginBottom: 4 }}>Filtered Debits</div>
          <div style={{ color: 'var(--red)', fontWeight: 700, fontFamily: 'var(--font-display)' }}>{fmt(totalDebit)}</div>
        </div>
        <div className="card-sm" style={{ flex: 1, minWidth: 140 }}>
          <div style={{ fontSize: '0.75rem', color: 'var(--text3)', marginBottom: 4 }}>Net Balance</div>
          <div style={{ color: totalCredit - totalDebit >= 0 ? 'var(--green)' : 'var(--red)', fontWeight: 700, fontFamily: 'var(--font-display)' }}>{fmt(Math.abs(totalCredit - totalDebit))} {totalCredit - totalDebit >= 0 ? 'CR' : 'DR'}</div>
        </div>
      </div>
      <div className="toolbar" style={{ flexWrap: 'wrap' }}>
        ...
        <select value={typeFilter} onChange={e => { setTypeFilter(e.target.value); setPage(1); }} style={{ minWidth: 120 }}>
          <option value="all">All Types</option>
          <option value="credit">Credit</option>
          <option value="debit">Debit</option>
        </select>
        <select value={personFilter} onChange={e => { setPersonFilter(e.target.value); setPage(1); }} style={{ minWidth: 160 }}>
        ...
        <input type="date" value={dateFrom} onChange={e => { setDateFrom(e.target.value); setPage(1); }} title="From date" style={{ minWidth: 140 }} />
        <input type="date" value={dateTo} onChange={e => { setDateTo(e.target.value); setPage(1); }} title="To date" style={{ minWidth: 140 }} />
        {(search || typeFilter !== 'all' || personFilter !== 'all' || dateFrom || dateTo) && (
          <button className="btn btn-secondary btn-sm" onClick={() => { setSearch(''); setTypeFilter('all'); setPersonFilter('all'); setDateFrom(''); setDateTo(''); setPage(1); }}>Clear</button>
        )}
      </div>
      <div className="table-wrap">
        <table>
          ...
          <tbody>
            {paged.length === 0 && (
              <tr><td colSpan={8}><div className="empty-state"><div className="empty-icon"><Icon name="history" size={28} /></div><div className="empty-title">No transactions found</div></div></td></tr>
            )}
            {paged.map(t => {
              const person = allPeople.find(p => p.id === t.personId);
              return (
                <tr key={t.id}>
                  ...
                  <td><span className={`badge ${t.type === 'credit' ? 'badge-green' : 'badge-red'}`}>{t.type === 'credit' ? 'Credit' : 'Debit'}</span></td>
                  <td>
                    <div style={{ fontWeight: 600 }}>{person?.name || 'Unknown'}</div>
                    <div style={{ fontSize: '0.72rem', color: 'var(--text3)', textTransform: 'capitalize' }}>{t.personType}</div>
                  </td>
                  ...
                  <td>
                    <div className="flex gap-2">
                      <button className="btn btn-icon btn-secondary btn-sm" title="View PDF" onClick={() => onViewPDF(t)}><Icon name="pdf" size={14} /></button>
                      <button className="btn btn-icon btn-secondary btn-sm" title="Edit" onClick={() => onEdit(t)}><Icon name="edit" size={14} /></button>
                      <button className="btn btn-icon btn-danger btn-sm" title="Delete" onClick={() => setDeleteTarget(t)}><Icon name="trash" size={14} /></button>
                    </div>
                  </td>
                </tr>
              );
      ...
      {totalPages > 1 && (
        <div className="flex items-center gap-2" style={{ marginTop: 12, justifyContent: 'center' }}>
          <button className="btn btn-secondary btn-sm" disabled={page === 1} onClick={() => setPage(p => p - 1)}>Prev</button>
          <span style={{ color: 'var(--text2)', fontSize: '0.85rem' }}>Page {page} of {totalPages}</span>
          <button className="btn btn-secondary btn-sm" disabled={page === totalPages} onClick={() => setPage(p => p + 1)}>Next</button>
        </div>
      )}
      {deleteTarget && (
        <ConfirmDialog
          message={`Delete this ${deleteTarget.type} note of ${fmt(deleteTarget.amount)}?`}
          onConfirm={() => { onDelete(deleteTarget.id); setDeleteTarget(null); }}
          onCancel={() => setDeleteTarget(null)}
        />
      )}
...
// ─── Reports ───────────────────────────────────────────────────────
function Reports({ transactions, customers, workers, companyName }) {
  ...
  const exportCSV = (rows, filename) => {
    ...
    const csvRows = [headers, ...rows.map(t => {
      const p = allPeople.find(x => x.id === t.personId);
      return [t.date, t.type, p?.name || '', t.amount, t.paymentMode, t.refNo, t.reason];
    })];
  ...
  const reportName = tab === 'monthly' ? `report_${selMonth}` : `report_${allPeople.find(p => p.id === selPerson)?.name || 'person'}`;
  ...
  const renderTable = (tx) => {
    ...
    return (
      <div className="table-wrap">
        <table>
          <thead>
            <tr><th>Date</th><th>Type</th><th>Person</th><th>Amount</th><th>Mode</th><th>Ref#</th><th>Description</th></tr>
          </thead>
          <tbody>
            {tx.sort((a, b) => b.date.localeCompare(a.date)).map(t => {
              ...
              return (
                <tr key={t.id}>
                  ...
                  <td>{p?.name || '-'}</td>
                  <td style={{ color: t.type === 'credit' ? 'var(--green)' : 'var(--red)', fontWeight: 600 }}>{fmt(t.amount)}</td>
                  <td style={{ textTransform: 'capitalize' }}>{t.paymentMode}</td>
                  <td style={{ color: 'var(--text3)' }}>{t.refNo || '-'}</td>
  ...
  return (
    <div>
      <div className="section-header">
        ...
        {exportTx && exportTx.length > 0 && (
          <button className="btn btn-secondary" onClick={() => exportCSV(exportTx, reportName + '.csv')}>
            <Icon name="excel" size={16} />Export CSV
          </button>
        )}
      </div>
      <div className="tabs">
      ...
      {tab === 'monthly' && (
        <div>
          <div className="toolbar">
            <input type="month" value={selMonth} onChange={e => setSelMonth(e.target.value)} style={{ minWidth: 160 }} />
          </div>
          <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
            <div className="card-sm" style={{ flex: 1 }}>
              <div style={{ fontSize: '0.75rem', color: 'var(--text3)', marginBottom: 4 }}>Total Credits</div>
              <div style={{ color: 'var(--green)', fontWeight: 700, fontFamily: 'var(--font-display)', fontSize: '1.2rem' }}>{fmt(monthlyReport.credits)}</div>
            </div>
            <div className="card-sm" style={{ flex: 1 }}>
              <div style={{ fontSize: '0.75rem', color: 'var(--text3)', marginBottom: 4 }}>Total Debits</div>
              <div style={{ color: 'var(--red)', fontWeight: 700, fontFamily: 'var(--font-display)', fontSize: '1.2rem' }}>{fmt(monthlyReport.debits)}</div>
            </div>
            <div className="card-sm" style={{ flex: 1 }}>
              <div style={{ fontSize: '0.75rem', color: 'var(--text3)', marginBottom: 4 }}>Transactions</div>
              <div style={{ fontWeight: 700, fontFamily: 'var(--font-display)', fontSize: '1.2rem' }}>{monthlyReport.tx.length}</div>
            </div>
          </div>
          {renderTable(monthlyReport.tx)}
        </div>
      )}
      ...
      {tab === 'person' && (
        <div>
          <div className="toolbar">
            <select value={selPerson} onChange={e => setSelPerson(e.target.value)} style={{ minWidth: 200 }}>
          ...
          {personReport && (
            <>
              <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
                <div className="card-sm" style={{ flex: 1 }}>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text3)', marginBottom: 4 }}>Total Credits</div>
                  <div style={{ color: 'var(--green)', fontWeight: 700, fontFamily: 'var(--font-display)', fontSize: '1.2rem' }}>{fmt(personReport.credits)}</div>
                </div>
                <div className="card-sm" style={{ flex: 1 }}>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text3)', marginBottom: 4 }}>Total Debits</div>
                  <div style={{ color: 'var(--red)', fontWeight: 700, fontFamily: 'var(--font-display)', fontSize: '1.2rem' }}>{fmt(personReport.debits)}</div>
                </div>
                <div className="card-sm" style={{ flex: 1 }}>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text3)', marginBottom: 4 }}>Balance</div>
                  <div style={{ color: personReport.credits - personReport.debits >= 0 ? 'var(--green)' : 'var(--red)', fontWeight: 700, fontFamily: 'var(--font-display)', fontSize: '1.2rem' }}>
                    {fmt(Math.abs(personReport.credits - personReport.debits))} {personReport.credits - personReport.debits >= 0 ? 'CR' : 'DR'}
                  </div>
                </div>
              </div>
              {renderTable(personReport.tx)}
            </>
          )}
          {!selPerson && <div className="empty-state"><div className="empty-icon"><Icon name="customers" size={28} /></div><div className="empty-title">Select a person to view their report</div></div>}
        </div>
      )}
    </div>
  );
...
function Settings({ data, onChange, addToast }) {
  ...
  const [pwForm, setPwForm] = useState({ username: data.currentUser?.username || '', oldPw: '', newPw: '', confirmPw: '' });
  const [pwError, setPwError] = useState('');

  const saveCompany = () => {
    onChange({ companyName: company.name, companyAddress: company.address, companyPhone: company.phone });
    addToast('Company settings saved!');
  };
  ...
  const changePw = () => {
    ...
    setPwError('');
    setPwForm(f => ({ ...f, oldPw: '', newPw: '', confirmPw: '' }));
    addToast('Password changed successfully!');
  };

  const exportAll = () => {
    const blob = new Blob([JSON.stringify({ customers: data.customers, workers: data.workers, transactions: data.transactions }, null, 2)], { type: 'application/json' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `ledgerflow_backup_${today()}.json`;
    a.click();
      ...
      <div className="grid-2">
        <div className="card">
          ...
          <div className="form-group" style={{ marginBottom: 12 }}>
            ...
            <input value={company.name} onChange={e => setCompany(c => ({ ...c, name: e.target.value }))} placeholder="Your Company Name" />
          </div>
          <div className="form-group" style={{ marginBottom: 12 }}>
            <label>Address</label>
            <textarea value={company.address} onChange={e => setCompany(c => ({ ...c, address: e.target.value }))} placeholder="Company address" rows={2} />
          </div>
          <div className="form-group" style={{ marginBottom: 16 }}>
            <label>Phone</label>
            <input value={company.phone} onChange={e => setCompany(c => ({ ...c, phone: e.target.value }))} placeholder="+91 ..." />
          </div>
        ...
        <div>
          <div className="card" style={{ marginBottom: 16 }}>
            ...
            <div className="form-group" style={{ marginBottom: 10 }}>
              ...
              <input type="password" value={pwForm.oldPw} onChange={e => setPwForm(f => ({ ...f, oldPw: e.target.value }))} />
            </div>
            <div className="form-group" style={{ marginBottom: 10 }}>
          ...
          <div className="card">
            <div style={{ fontWeight: 700, marginBottom: 12 }}>Data Management</div>
            <div style={{ color: 'var(--text2)', fontSize: '0.875rem', marginBottom: 14 }}>Export all data as a JSON backup file.</div>
            <button className="btn btn-secondary" onClick={exportAll}><Icon name="excel" size={16} />Export Backup</button>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <div style={{ color: 'var(--text2)', fontSize: '0.875rem' }}>Welcome back — here's your financial overview</div>
      </div>

      <div className="stats-grid">
        <div className="stat-card green">
          <div className="stat-icon green"><Icon name="arrowUp" size={18} color="var(--green)" /></div>
          <div className="stat-label">Total Credits</div>
          <div className="stat-value green">{fmt(totalCredits)}</div>
          <div className="stat-sub">{transactions.filter(t => t.type === 'credit').length} transactions</div>
        </div>
        <div className="stat-card red">
          <div className="stat-icon red"><Icon name="arrowDown" size={18} color="var(--red)" /></div>
          <div className="stat-label">Total Debits</div>
          <div className="stat-value red">{fmt(totalDebits)}</div>
          <div className="stat-sub">{transactions.filter(t => t.type === 'debit').length} transactions</div>
        </div>
        <div className="stat-card amber">
          <div className="stat-icon amber"><Icon name="coin" size={18} color="var(--amber)" /></div>
          <div className="stat-label">Net Balance</div>
          <div className="stat-value amber">{fmt(Math.abs(totalCredits - totalDebits))}</div>
          <div className="stat-sub">{totalCredits - totalDebits >= 0 ? 'Credit surplus' : 'Debit surplus'}</div>
        </div>
        <div className="stat-card blue">
          <div className="stat-icon blue"><Icon name="customers" size={18} color="var(--blue)" /></div>
          <div className="stat-label">People</div>
          <div className="stat-value blue">{customers.length + workers.length}</div>
          <div className="stat-sub">{customers.length} customers · {workers.length} workers</div>
        </div>
      </div>
      ...
      <div className="grid-2">
        <div className="card">
          ...
          <div className="bar-chart">
            {chartMonths.map(m => (
              <div key={m.key} className="bar-group" style={{ flexDirection: 'column', alignItems: 'center', gap: 2, justifyContent: 'flex-end' }}>
                <div style={{ width: '100%', display: 'flex', alignItems: 'flex-end', gap: 2, height: '100%' }}>
                  <div className="bar bar-credit" style={{ height: `${(m.credits / maxVal) * 100}%` }} title={`Credits: ${fmt(m.credits)}`} />
                  <div className="bar bar-debit" style={{ height: `${(m.debits / maxVal) * 100}%` }} title={`Debits: ${fmt(m.debits)}`} />
                </div>
              </div>
            ))}
          </div>
          <div className="bar-labels">
            {chartMonths.map(m => <div key={m.key} className="bar-label">{m.label}</div>)}
          </div>
          <div style={{ display: 'flex', gap: 16, marginTop: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.75rem', color: 'var(--text2)' }}><div style={{ width: 10, height: 10, borderRadius: 2, background: 'var(--green)' }} />Credits</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.75rem', color: 'var(--text2)' }}><div style={{ width: 10, height: 10, borderRadius: 2, background: 'var(--red)' }} />Debits</div>
          </div>
        </div>

        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div style={{ fontWeight: 700, fontSize: '0.95rem' }}>Top Customers by Balance</div>
          </div>
          {topCustomers.length === 0 ? (
            <div className="empty-state" style={{ padding: 24 }}><div className="empty-sub">No customers yet</div></div>
          ) : topCustomers.map(c => (
            <div key={c.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid var(--border)' }}>
              <div>
                ...
                <div style={{ fontSize: '0.75rem', color: 'var(--text3)' }}>{fmt(c.credits)} CR · {fmt(c.debits)} DR</div>
              </div>
              <span className={`badge ${c.balance >= 0 ? 'badge-green' : 'badge-red'}`}>{fmt(Math.abs(c.balance))} {c.balance >= 0 ? 'CR' : 'DR'}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="card mt-4">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          <button className="btn btn-secondary btn-sm" onClick={() => setPage('history')}>View All</button>
        </div>
        {recentTx.length === 0 ? (
          <div className="empty-state" style={{ padding: 24 }}><div className="empty-sub">No transactions yet. Add your first credit or debit note.</div></div>
        ) : (
          <div className="table-wrap" style={{ border: 'none' }}>
            <table>
              <thead><tr><th>Date</th><th>Type</th><th>Person</th><th>Amount</th><th>Mode</th></tr></thead>
              <tbody>
                {recentTx.map(t => {
                  const p = allPeople.find(x => x.id === t.personId);
                  return (
                    <tr key={t.id}>
                      ...
                      <td style={{ color: t.type === 'credit' ? 'var(--green)' : 'var(--red)', fontWeight: 700 }}>{fmt(t.amount)}</td>
                      <td style={{ color: 'var(--text3)', textTransform: 'capitalize' }}>{t.paymentMode}</td>
                    </tr>
                  );
                })}
              </tbody>
...
export default function App() {
  ...
  // Modals
  const [showPersonForm, setShowPersonForm] = useState(null); // {type, person}
  const [showTxForm, setShowTxForm] = useState(null); // {type, tx}
  ...
  if (!data.currentUser) {
    return (
      <>
        <style>{styles}</style>
        <LoginPage users={data.users} onLogin={user => updateData({ currentUser: user })} />
      </>
    );
  }
  ...
  const addCustomer = (form) => {
    const c = { ...form, id: uid(), createdAt: Date.now() };
    updateData({ customers: [...data.customers, c] });
    addToast('Customer added!');
    setShowPersonForm(null);
  };
  const editCustomer = (form) => {
    updateData({ customers: data.customers.map(c => c.id === form.id ? { ...c, ...form } : c) });
    addToast('Customer updated!');
    setShowPersonForm(null);
  };
  const deleteCustomer = (id) => { updateData({ customers: data.customers.filter(c => c.id !== id) }); addToast('Customer deleted.', 'error'); };

  const addWorker = (form) => {
    const w = { ...form, id: uid(), createdAt: Date.now() };
    updateData({ workers: [...data.workers, w] });
    addToast('Worker added!');
    setShowPersonForm(null);
  };
  const editWorker = (form) => {
    updateData({ workers: data.workers.map(w => w.id === form.id ? { ...w, ...form } : w) });
    addToast('Worker updated!');
    setShowPersonForm(null);
  };
  const deleteWorker = (id) => { updateData({ workers: data.workers.filter(w => w.id !== id) }); addToast('Worker deleted.', 'error'); };

  const saveTx = (form) => {
    if (form.id) {
      updateData({ transactions: data.transactions.map(t => t.id === form.id ? { ...t, ...form } : t) });
      addToast('Transaction updated!');
    } else {
      const t = { ...form, id: uid(), createdAt: Date.now() };
      updateData({ transactions: [...data.transactions, t] });
      addToast(`${form.type === 'credit' ? 'Credit' : 'Debit'} note saved!`);
    }
    setShowTxForm(null);
  };
  const deleteTx = (id) => { updateData({ transactions: data.transactions.filter(t => t.id !== id) }); addToast('Transaction deleted.', 'error'); };
  ...
  const pageTitles = { dashboard: 'Dashboard', customers: 'Customers', workers: 'Workers', credit: 'New Credit Note', debit: 'New Debit Note', history: 'Transaction History', reports: 'Reports', settings: 'Settings' };
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
          <div className="sidebar-logo">
            ...
            <div>
              ...
              <div className="sidebar-logo-sub">Notes Manager</div>
            </div>
          </div>
          <nav className="sidebar-nav">
            {navItems.slice(0, 5).map(item => (
              <div key={item.id} className={`nav-item ${page === item.id && item.id !== 'credit' && item.id !== 'debit' ? 'active' : ''}`} onClick={() => handleNavClick(item.id)}>
          ...
          <div className="sidebar-footer">
            <div className="nav-item" onClick={() => updateData({ currentUser: null })}>
        ...
        <div className="main">
          <header className="header">
            ...
            <div className="header-actions">
              ...
              <div className="user-badge">
                <div className="user-avatar">{data.currentUser.username[0].toUpperCase()}</div>
                <span style={{ display: 'none' }} className="username">{data.currentUser.username}</span>
              </div>
            </div>
          </header>
          <div className="page-content">
            ...
            {page === 'customers' && (
              <PeopleList
                type="customer" data={data.customers} transactions={data.transactions}
                onAdd={() => setShowPersonForm({ type: 'Customer', person: null })}
                onEdit={(p) => setShowPersonForm({ type: 'Customer', person: p })}
                onDelete={deleteCustomer}
                onViewHistory={(p) => { setHistoryFilter(p.id); setPage('history'); }}
              />
            )}
            {page === 'workers' && (
              <PeopleList
                type="worker" data={data.workers} transactions={data.transactions}
                onAdd={() => setShowPersonForm({ type: 'Worker', person: null })}
                onEdit={(p) => setShowPersonForm({ type: 'Worker', person: p })}
            ...
            {page === 'history' && (
              <TransactionHistory
                transactions={data.transactions} customers={data.customers} workers={data.workers}
                filterPersonId={historyFilter}
                onEdit={(t) => setShowTxForm({ type: t.type, tx: t })}
                onDelete={deleteTx}
                onViewPDF={(t) => setShowPDF(t)}
              />
            )}
            {page === 'reports' && <Reports transactions={data.transactions} customers={data.customers} workers={data.workers} companyName={data.companyName} />}
      ...
      {showPersonForm && (
        <PersonForm
          type={showPersonForm.type}
          initial={showPersonForm.person}
          onSave={(form) => {
            if (showPersonForm.type === 'Customer') {
              showPersonForm.person ? editCustomer({ ...showPersonForm.person, ...form }) : addCustomer(form);
            } else {
              showPersonForm.person ? editWorker({ ...showPersonForm.person, ...form }) : addWorker(form);
            }
          }}
      ...
      {showTxForm && (
        <TransactionForm
          type={showTxForm.type}
          customers={data.customers}
          workers={data.workers}
          initial={showTxForm.tx}
          onSave={saveTx}
